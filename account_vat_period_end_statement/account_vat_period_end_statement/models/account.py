...
class account_vat_period_end_statement(orm.Model):
    ...
    def _get_statement_from_move(self, cr, uid, ids, context=None):
        move = {}
        statement_ids = []
        for move in self.pool.get('account.move').browse(cr, uid, ids,
                                                         context=context):
    ...
    _columns = {
        ...
        'authority_vat_account_id': fields.many2one(
            ...
            states={'confirmed': [('readonly', True)],
                    'paid': [('readonly', True)],
                    'draft': [('readonly', False)]}),
        'authority_vat_amount': fields.function(_compute_authority_vat_amount,
                                                method=True,
                                                string='Authority VAT Amount'),
        'payable_vat_amount': fields.function(_compute_payable_vat_amount,
                                              method=True,
        ...
        'date': fields.date('Date', required=True,
                            states={'confirmed': [('readonly', True)],
                                    'paid': [('readonly', True)],
                                    'draft': [('readonly', False)]}),
        'move_id': fields.many2one('account.move', 'VAT statement move',
                                   readonly=True),
        ...
        'reconciled': fields.function(
            ...
            store={'account.vat.period.end.statement': (lambda self, cr, uid,
                                                        ids, c={}: ids,
                                                        None, 50),
        ...
        'period_ids': fields.one2many('account.period', 'vat_statement_id',
                                      'Periods'),
        ...
        for statement in self.browse(cr, uid, ids, context):
            if statement.state == 'confirmed' or statement.state == 'paid':
                raise orm.except_orm(_('Error!'), _(
                    'You cannot delete a confirmed or paid statement'))
    ...
    def create_move(self, cr, uid, ids, context=None):
        ...
        for statement in self.browse(cr, uid, ids, context):
            period_ids = period_obj.find(cr, uid, dt=statement.date,
                                         context=context)
            if len(period_ids) != 1:
                raise orm.except_orm(
                    _('Encoding error'),
                    _('No period found or more than one period '
            ...
            if statement.authority_vat_amount > 0:
                ...
                if statement.payment_term_id:
                    due_list = term_pool.compute(
                        cr, uid, statement.payment_term_id.id,
    ...
    def compute_amounts(self, cr, uid, ids, context=None):
        ...
        for statement in self.browse(cr, uid, ids, context):
            ...
            if prev_statement_ids:
                prev_statement = self.browse(cr, uid, prev_statement_ids[
                    len(prev_statement_ids) - 1], context)
                if prev_statement.residual > 0 \
            ...
            for debit_tax_code_id in debit_tax_code_ids:
                debit_tax_code = tax_code_pool.browse(cr, uid,
                                                      debit_tax_code_id,
                                                      context)
                total = 0.0
                for period in statement.period_ids:
                    context['period_id'] = period.id
                    total += tax_code_pool.browse(cr, uid, debit_tax_code_id,
                                                  context).sum_period
            ...
            for credit_tax_code_id in credit_tax_code_ids:
                credit_tax_code = tax_code_pool.browse(cr, uid,
                                                       credit_tax_code_id,
                                                       context)
                total = 0.0
                for period in statement.period_ids:
                    context['period_id'] = period.id
                    total += tax_code_pool.browse(cr, uid, credit_tax_code_id,
                                                  context).sum_period
...
class statement_generic_account_line(orm.Model):
    ...
    def on_change_vat_account_id(self, cr, uid, ids, vat_account_id=False,
                                 context=None):
        res = {}
        res['value'] = {}
...
class account_tax_code(orm.Model):
    ...
    _columns = {
        'vat_statement_account_id': fields.many2one(
            ...
            The tax code balance will be associated to \
            this account after selecting the period in VAT statement"),
...
class account_period(orm.Model):
    ...
    _columns = {
        'vat_statement_id': fields.many2one('account.vat.period.end.statement',
                                            "VAT statement"),
    }
