...
class vat_period_end_statement_report(report_sxw.rml_parse):
    ...
    def __init__(self, cr, uid, name, context=None):
        if context is None:
            context = {}
        super(vat_period_end_statement_report, self).__init__(cr, uid, name, context=context)
        self.query = ""
    ...
    def _get_statement(self, statement_id):
        ...
        statement = False
        if statement_id:
            statement = statement_obj.browse(self.cr, self.uid, 
                                         statement_id, 
                                         self.context)
        return statement
    ...
    def _get_account_vat_amounts(self, type='credit', statement_account_line=[], context=None):
        ...
        if type == 'credit':
            ...
            raise orm.except_orm(_('Error'), _('Type account neither credit or debit !'))
...
class report_salecatalogbase(orm.AbstractModel):
    _name = 'report.account_vat_period_end_statement_report.report_vatperiodendstatement'
    _inherit = 'report.abstract_report'
    _template = 'account_vat_period_end_statement_report.report_vatperiodendstatement'
